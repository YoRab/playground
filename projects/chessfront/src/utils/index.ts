...
const getMoveIfFree = ({
  ...
  return {
    move:
      canTake && targetPiece?.color !== color
        ? [targetRow, targetCol]
        : undefined,
    keep: false,
  };
...
const getPawnMoves = (
  ...
  const [checkPos, checkPosNext, direction] =
    color === "white"
      ? [() => position[0] > 0, () => position[0] === 6, -1]
      : [() => position[0] < 7, () => position[0] === 1, 1];
...
const getQueenMoves = (
  ...
  return [
    ...getRookMoves(board, pieces, active),
    ...getBishopMoves(board, pieces, active),
  ];
};
...
const isThereChess = (
  ...
  const kingPos = pieces.find(
    (piece) => piece.id === `${color === "white" ? "w" : "b"}king`
  )?.position;
  if (!kingPos) return false;
  const ennemyPieces = pieces.filter(
    (piece) => piece.color === (color === "white" ? "black" : "white")
  );
  for (const piece of ennemyPieces) {
    const pieceMoves = getPiecesMoves(board, pieces, piece);
    for (const ennemyMove of pieceMoves) {
      if (ennemyMove[0] === kingPos[0] && ennemyMove[1] === kingPos[1])
        return true;
    }
  }
  return false;
